...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($SetNumber) && !empty($MinReps) && !empty($MaxReps) && !empty($MinWeight) && !empty($MaxWeight) && !empty($WorkoutID)) {
        // Construct the SQL query to insert weight training information
        $trainingQuery = "INSERT INTO WeightTraining (SetNumber, MinReps, MaxReps, MinWeight, MaxWeight, WorkoutID) VALUES (?, ?, ?, ?, ?, ?)";
        ...
        if ($trainingStmt === false) {
            ...
            $trainingStmt->bind_param("iidddi", $SetNumber, $MinReps, $MaxReps, $MinWeight, $MaxWeight, $WorkoutID);
