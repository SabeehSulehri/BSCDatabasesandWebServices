...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($AdvancedTechnique) && !empty($Prerequisite) && !empty($PersonalRecordsChart) && !empty($TargetedMuscle) && !empty($WorkoutID)) {
        // Construct the SQL query to insert advanced workout information
        $advancedQuery = "INSERT INTO Advanced (AdvancedTechnique, Prerequisite, PersonalRecordsChart, TargetedMuscle, WorkoutID) VALUES (?, ?, ?, ?, ?)";
        ...
        if ($advancedStmt === false) {
            ...
            $advancedStmt->bind_param("dddsi", $AdvancedTechnique, $Prerequisite, $PersonalRecordsChart, $TargetedMuscle, $WorkoutID);
