...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($AerobicClass) && !empty($CaloriesBurned) && !empty($RunningTrack) && !empty($CardioMachines) && !empty($WorkoutID)) {
        // Construct the SQL query to insert cardio information
        $cardioQuery = "INSERT INTO Cardio (AerobicClass, CaloriesBurned, RunningTrack, CardioMachines, WorkoutID) VALUES (?, ?, ?, ?, ?)";
        ...
        if ($cardioStmt === false) {
            ...
            $cardioStmt->bind_param("dddsi", $AerobicClass, $CaloriesBurned, $RunningTrack, $CardioMachines, $WorkoutID);
